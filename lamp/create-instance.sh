...
wget  https://raw.githubusercontent.com/praveensiddu/aws/main/utils/create-secgrp.sh lamp-secgrp
aws ec2 run-instances --image-id $AMZLINUZ2_AMI --count 1 --instance-type t2.micro --key-name $MYSSHKEYNAME --user-data cloud-init.sh  --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=lamp}]"  --security-groups lamp-secgrp
